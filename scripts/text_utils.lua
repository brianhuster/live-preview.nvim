...
local function render_md(node, start_indent, indent, text_width, level, is_list)
	...
	elseif ntype == "list_item" then
		...
		for i, child in ipairs(node) do
			...
			vim.list_extend(parts, render_md(child, sindent, indent + offset, text_width, level + 1, true))
		end
	else
		if node.text then
		...
		for i, child in ipairs(node) do
			...
			vim.list_extend(parts, render_md(child, start_indent0, indent, text_width, level + 1, is_list))
